...
if ( $has_preview AND ! post_password_required() ) {
	...
	if ( in_array( $layout, array( 'classic', 'flat' ) ) || ( $layout == 'cards' AND $post_format == 'gallery' ) ) {
		$featured_html = us_get_post_preview( $the_content, $strip_from_the_content );
	}
...
if ( $post_format == 'gallery' ) {
  // Trim gallery tags and generate an excerpt - because if captions are turned on, they get to the excerpt body //
  $the_content = preg_replace( '~\[us_gallery.+?\]|\[us_image_slider.+?\]|\[gallery.+?\]~', '', $the_content );
